...
async function main() {
  ...
  const questions = [
    ...
    },
    {
      ...
      label: 'O que você mais gostaria de conquistar com um programa de apoio profissional no seu bairro?',
    ...
    {
      key: 'influencia_decisao',
      label: 'Quando você pensa em buscar uma oportunidade profissional, quem mais influencia sua decisão?',
      type: QuestionType.SINGLE_CHOICE,
      required: true,
      options: [
        'Minha família',
    ...
    {
      key: 'fator_permanencia',
      label: 'O que mais ajudaria você a continuar participando do Gente Daqui?',
      type: QuestionType.MULTIPLE_CHOICE,
      required: true,
      options: [
    ...
    {
      key: 'frequencia_acompanhamento',
      label: 'Com que frequência você gostaria de receber acompanhamento?',
      type: QuestionType.SINGLE_CHOICE,
      required: true,
      options: ['Toda semana', 'A cada 15 dias', 'Uma vez por mês', 'Somente quando eu precisar'],
      maxSelections: null,
      section: 6,
      position: 18,
    },
